...
1. **加密模拟支持**
   - 使用 Python `hashlib`、`hmac`、`pycryptodome` 库模拟常见 Java 加密方法
   - 支持 MD5、SHA256、AES、DES 等常用加密算法
   - 兼容阅读 App 的 Java 加密调用方式

2. **WebView 替代方案**
   - 集成 Playwright/Puppeteer 作为 headless browser
   - 支持 `java.webView` 动态渲染场景
   - 可配置是否启用（增加资源消耗）
